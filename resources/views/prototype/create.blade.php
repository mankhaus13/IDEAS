...
<head>
    ...
    <title>Создание нового прототипа</title>
...
<div class="w-full max-w-md space-y-8">
    ...
    <div class="mt-8 bg-white dark:bg-gray-800 shadow overflow-hidden sm:rounded-lg">
        <div class="px-6 py-8">
            <form method="POST" action="{{ route('prototype.store') }}" class="space-y-6">
                @csrf
                ...
                <div>
                    ...
                    <div class="mt-1">
                        <select id="status" name="status" required
                            ...
                            <option value="success" {{ old('status') == 'success' ? 'selected' : '' }}>Успешно</option>
                            <option value="warning" {{ old('status') == 'warning' ? 'selected' : '' }}>Требует доработки</option>
                            <option value="danger" {{ old('status') == 'danger' ? 'selected' : '' }}>Провален</option>
                            <option value="info" {{ old('status') == 'info' ? 'selected' : '' }}>В процессе</option>
                        </select>
                        @error('status')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                ...
                <div>
                    ...
                    <div class="mt-1">
                        <textarea id="description" name="description" rows="3"
                                  class="block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-white"
                                  placeholder="Детальное описание эксперимента">{{ old('description') }}</textarea>
                        @error('description')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
